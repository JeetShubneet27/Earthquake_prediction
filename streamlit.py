...
WEATHER_API_KEY = 'f8ca3c4d1b9d47a880581145251604'
...
def fetch_realtime_data():
    try:
        ...
        return pd.DataFrame(quake_data)
    except Exception as e:
...
def get_region_name(lat, lon):
    if 15 <= lat <= 90 and -168 <= lon <= -52:
        return "Northern North America" if lat > 50 else "Central North America"
...
def load_data(use_realtime=False):
    ...
    if use_realtime:
        ...
        if realtime_df is not None:
            ...
            combined_df['cluster_name'] = combined_df.apply(lambda row: get_region_name(row['latitude'], row['longitude']), axis=1)
            return combined_df
    historical_df['cluster_name'] = historical_df.apply(lambda row: get_region_name(row['latitude'], row['longitude']), axis=1)
    return historical_df

def predict_probability(lat, lon, depth, date_str, model, scaler, features, df):
    try:
        date = pd.to_datetime(date_str, format='mixed', utc=True)
    except:
        ...
        if app_mode == "Prediction":
            ...
            with st.form("prediction_form"):
                ...
                with col2:
                    ...
                    date = st.date_input("Date (IST)", value=now_ist.date())
    # Always use current IST time (no time input shown)
                    dt_ist = india.localize(datetime.combine(date, now_ist.time()))
                    dt_utc = dt_ist.astimezone(pytz.UTC)
                    date_str = dt_utc.strftime("%Y-%m-%d %H:%M:%S%z")
                    st.write(f"Prediction will use current IST time: {dt_ist.strftime('%Y-%m-%d %I:%M %p')}")
                        ...
                        try:
                            ...
                            if location:
                                ...
                                if weather:
                                    ...
                                    col1w, col2w = st.columns([1,2])
                                    with col1w:
                                        st.image(f"https:{weather['icon']}", width=60)
                                    with col2w:
                                        ...
                                        st.write(f"**Conditions:** {weather['condition']}")
                        except Exception as e:
                ...
                if st.form_submit_button("Predict Magnitude"):
                    with st.spinner("Generating prediction..."):
                        ...
                        with res_col:
                            ...
                            st.markdown(f"### Predicted Magnitude: <span style='color:{mag_color}'>{predicted_mag:.1f}</span>", unsafe_allow_html=True)
                            ...
                            if not similar_events.empty:
                                st.dataframe(similar_events[['time', 'mag', 'depth', 'cluster_name']]
                                    .rename(columns={'time': 'Date', 'mag': 'Magnitude', 'depth': 'Depth', 'cluster_name': 'Region'}))
                        
                        with map_col:
                            pred_map = folium.Map(location=[lat, lon], zoom_start=8)
                            folium.Marker([lat, lon], popup=f"Predicted: M{predicted_mag:.1f}",
                                        icon=folium.Icon(color=mag_color, icon='exclamation-triangle')).add_to(pred_map)
                            
                            weather = get_weather_data(lat, lon)
                            if weather:
                                folium.Marker([lat, lon], popup=f"""
                                    <b>Weather Conditions</b><br>
                                    Temp: {weather['temp']}°C<br>
                                    Feels Like: {weather['feels_like']}°C<br>
                                    Wind: {weather['wind_speed']} km/h<br>
                                    {weather['condition']}
                                    """, icon=folium.Icon(color='blue', icon='cloud')).add_to(pred_map)
                            
                            cluster_data = df[df['cluster_name'] == cluster_name]
                            for _, row in cluster_data[cluster_data['mag'] > 4.0].iterrows():
                                folium.CircleMarker([row['latitude'], row['longitude']], radius=row['mag'],
                                                  color='orange', fill=True, popup=f"M{row['mag']} {row['cluster_name']}").add_to(pred_map)
                            
                            folium_static(pred_map)

        elif app_mode == "Early Warnings":
            ...
            - WeatherAPI integration for environmental context
